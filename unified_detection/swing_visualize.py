"""
Unified swing detection — event-based approach (full swings + irregular).
...
Produces 2 HTML files:
  - irregular.html
  - full_swings.html

Usage:
    python unified_detection/swing_visualize.py     # from project root
"""
...
BS_TOP_MIN_DISTANCE_SAMPLES = 15    # min samples between detected valleys (~0.3s at 50Hz)
...
FS_MIN_DISTANCE_SAMPLES  = 15    # min samples between peaks (~0.3s at 50Hz)
...
def main():
    ...
    irreg_files = sorted(glob.glob(os.path.join(_ROOT, 'iregular_samples', 'motion_test_*.csv')))
    full_files = sorted(glob.glob(os.path.join(_ROOT, 'full_swing_samples', 'motion_test_*.csv')))

    print(f'Swing detection visualization')
    print(f'  Irregular samples: {len(irreg_files)}')
    print(f'  Full swing samples: {len(full_files)}')
    print()

    if irreg_files:
        print('Generating irregular.html...')
        plot_stitched(irreg_files, os.path.join(out_dir, 'irregular.html'),
                      'Irregular Samples')

    if full_files:
        print('Generating full_swings.html...')
        plot_stitched(full_files, os.path.join(out_dir, 'full_swings.html'),
                      'Full Swing Samples')
